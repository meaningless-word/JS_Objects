...
// обобщенный конструктор класса "электроприборы"
function Device(name, manufacturer, model, power) {
    this.name = name;
  ...
  // вынесенные за пределы функции-конструктора методы
  // обеспечивают экономичный расход памяти
  ...
  Device.prototype.powerСonsumption = function() {
  ...
  // создадим подкласс техники - устройства со встроенным аккумулятором
  function RechargeableDevice(name, manufacturer, model, power, batteryCapacity){
    Device.call(this, name, manufacturer, model, power); 
    this.batteryCapacity = batteryCapacity; //ёмкость батареи
    this.isCharging = false;
    this.batteryLevel = Math.random() * 100;
  }
  RechargeableDevice.prototype = new Device();
  
  // предположим, что между событиями включения (switchOn) и выключения (switchOff)
  // происходит расход заряда батареи,
  // а потребление энергии происходит только когда устройство заряжается - подключено к сети (plug)
  // а во время отключения от зарядки (unplug) получает случайную порцию заряда
  RechargeableDevice.prototype.plug = function() {
    this.isSwitchedOn = false;
    this.isCharging = true;
  }
  
  RechargeableDevice.prototype.unplug = function() {
    this.isCharging = false;
  ...
  RechargeableDevice.prototype.powerСonsumption = function() {
    return this.isCharging ? this.power : 0; //потребление тлько во время зарядки
  }
  
  RechargeableDevice.prototype.switchOn = function() {
    this.unplug();
  ...
  RechargeableDevice.prototype.switchOff = function() {
  ...
  RechargeableDevice.prototype.showState = function() {
    if (this.isCharging) {
